...
MemoryPrefetcher::MemoryPrefetcher(std::map<std::string, std::string>& backend_options, std::vector<int>::iterator prefetch_start,
    ...
    if (eviction_policy == 0) {
      ...
      printf("Number of cache samples per process %d\n", num_elems);fflush(stdout);
    }
    ...
    if (eviction_policy == 0)
    {
      ...
      file_ends.resize(std::min(backend->get_length(), int(capacity/max_file_size)), 0);
      printf("Number of cache samples per process %d\n", int(capacity/max_file_size));fflush(stdout);
...
void MemoryPrefetcher::fetch_and_rm_cache(int file_id, char* dst) {
  ...
  int next_idx;
  printf("TW: in fetch_and_rm_cache call before create lock, file_id = %d\n", file_id); fflush(stdout);
  std::unique_lock<std::mutex> lock(prefetcher_mutex);
  printf("TW: in fetch_and_rm_cache call after create lock, file_id = %d\n", file_id); fflush(stdout);
  if (file_cached.count(file_id) == 0 || file_cached[file_id] == 0) {
    ...
    if (file_cached.size() * max_file_size + max_file_size >= capacity) {
      printf("TW: in fetch_and_rm_cache call, outer if with file_cached[file_id] == 0 or not there, cache full, file_id = %d\n", file_id); fflush(stdout);
      ...
      printf("TW: in fetch_and_rm_cache call, outer if with file_cached[file_id] == 0 or not there, cache not full, start, file_id = %d\n", file_id); fflush(stdout);
      // cache is not full, set next_idx to buffer_offset
      cache_file = true;
      next_idx = buffer_offset;
      buffer_offset += 1;
      file_cached[file_id] = 0;
      printf("TW: in fetch_and_rm_cache call, outer if with file_cached[file_id] == 0 or not there, cache not full, end, file_id = %d\n", file_id); fflush(stdout);
      // std::cout << "cache is not full & file_id is " << file_id << "idx is " << next_idx << std::endl;
    }
  } else if (file_cached[file_id] == 1) {
    printf("TW: in fetch_and_rm_cache call, outer if with file_cached[file_id] == 1, file_id = %d\n", file_id); fflush(stdout);
    // A different thread already fetched this file.
    ...
    printf("TW: in fetch_and_rm_cache call, outer if with file_cached[file_id] == 2, file_id = %d\n", file_id); fflush(stdout);
    // A different thread is currently caching this file, wait.
  ...
  printf("TW: in fetch_and_rm_cache call first stage finished, file_id = %d\n", file_id); fflush(stdout);
  if (cache_file) {
  ...
  printf("TW: in fetch_and_rm_cache call second stage finished with cache_file = %d, file_id = %d\n", cache_file, file_id); fflush(stdout);

  // Fetch without the lock.
  if (cache_file) {
    // if cache this file
    // int idx = file_id_to_idx[file_id];
    printf("TW: in fetch_and_rm_cache call, third stage, p1, file_id = %d, next_idx = %d, \n", file_id, next_idx); fflush(stdout);
    ...
    printf("TW: in fetch_and_rm_cache call, third stage, p2, file_id = %d, start = %llu, end = %llu\n", file_id, start, end); fflush(stdout);
    backend->fetch(file_id, buffer + start);
    printf("TW: in fetch_and_rm_cache call, third stage, p3, file_id = %d\n", file_id); fflush(stdout);
    // Copy to the destination buffer.
    memcpy(dst, buffer + start, end - start);
    printf("TW: in fetch_and_rm_cache call, third stage, p4, file_id = %d\n", file_id); fflush(stdout);
  } else {
    // cache is full and this file will not be cached (just add to the sbf)
    backend->fetch(file_id, dst);
  }
  printf("TW: in fetch_and_rm_cache call third stage finished, file_id = %d\n", file_id); fflush(stdout);
  // Reclaim the lock to mark the file as cached.
  lock.lock();
  if (cache_file) {
    file_cached[file_id] = 1;
  ...
  printf("TW: in fetch_and_rm_cache call four stage finished, file_id = %d\n", file_id); fflush(stdout);
  lock.unlock();
  if (cache_file) {
    metadata_store->insert_cached_file(storage_level, file_id);
    prefetch_cv.notify_all();
  }
  printf("TW: in fetch_and_rm_cache call all finished, file_id = %d\n", file_id); fflush(stdout);
}
